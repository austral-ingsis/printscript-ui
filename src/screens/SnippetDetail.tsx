...
export const SnippetDetail = (props: SnippetDetailProps) => {
  ...
  const [snippetOutput, setSnippetOutput] = useState<string[]>([])
  ...
  const {mutate: runSnippet, isLoading: isRunSnippetLoading, data: result, error} = useRunSnippet()
          ...
          isLoading ? (<>
            ...
            <Box display="flex" flexDirection="row" gap="8px" padding="8px">
              ...
              <Tooltip title={"Format"}>
                <IconButton onClick={() => formatSnippet(snippet)} disabled={isFormatLoading}>
                  <ReadMoreIcon />
                </IconButton>
              </Tooltip>
              <Tooltip title={"Save changes"}>
                <IconButton color={"primary"} onClick={() => updateSnippet({id: id, content: code, name: snippet.name})} disabled={isUpdateSnippetLoading || snippet?.content === code} >
                  <Save />
                </IconButton>
              </Tooltip>
            ...
            <Box pt={1} flex={1} marginTop={2}>
              ...
              <SnippetExecution snippetOuput={snippetOutput}/>
            </Box>
          </>
        }
